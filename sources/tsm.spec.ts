...
// -------- Contract SDK --------
import { TSM, Mint, TokenTransfer } from "./output/tsm_TSM";
import { JettonDefaultWallet, TokenBurn } from "./output/tsm_JettonDefaultWallet";
...
describe("contract", () => {
    ...
    beforeAll(async () => {
        ...
        token = blockchain.openContract(await TSM.fromInit(deployer.address, content, max_supply));
    ...
    it("should transfer successfully", async () => {
        ...
        const cellPayload = beginCell().storeUint(0, 1).storeUint(0, 32).endCell();

        // convert Cell to Slice
        const slicePayload = cellPayload.beginParse();
    ...
    it("Onchian Testing: DeDust", async () => {
        ...
        const jettonRoot = blkch.openContract(await TSM.fromInit(player.address, content, max_supply));
